## cosmology
[![Build Status](https://travis-ci.org/uwescience/cosmology.svg?branch=master)](https://travis-ci.org/uwescience/cosmology)
...
instructions at the [bottom of this page](#using-cosmology-as-a-template).
...
`cosmology/cosmology.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `cosmology.py` in directory called
`cosmology`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import cosmology as sb` in an
interactive Python session, the classes and functions defined inside of the
`cosmology.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the cosmology module, the main classes defined at the bottom of
...
Either way, you can create a `cosmology/data` folder in which you can
...
(typically, something like our `cosmology/tests/test_cosmology.py`). Within each
...
have one such test in `cosmology/tests/test_cosmology.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/cosmology`),
and type:
...
will see a message such as:
    ...
    cosmology/tests/test_cosmology.py .F...
    ...
    cosmology/tests/test_cosmology.py:49: AssertionError
...
To document `cosmology` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/cosmology/blob/master/doc/_static/RTD-advanced-conf.png)

 http://cosmology.readthedocs.org/en/latest/
...
A `cosmology/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/cosmology) for the software. This
...
`cosmology` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `cosmology`, we use the
...
[here](https://github.com/uwescience/cosmology/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/cosmology
...
`cosmology/cosmology.py` to be called `smallish/smallish.py`

	git mv cosmology smallish
	git mv smallish/cosmology.py smallish/smallish.py
	git mv smallish/tests/test_cosmology.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `cosmology` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `cosmology` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `cosmology` in them. Type
the following to see where all these files are:

	git grep cosmology

You can replace `cosmology` for `smallish` quickly with:

	git grep -l 'cosmology' | xargs sed -i 's/cosmology/smallish/g'
...
Edit all the mentions of `cosmology` in the `cosmology/__init__.py` file, and in
the `cosmology/version.py` file as well.
